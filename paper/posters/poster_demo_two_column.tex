...
\newcommand{\takeawaybox}[1]{%
  ...
  \makebox[\linewidth][c]{%
    \colorbox{PosterGray}{%
      \parbox{0.97\linewidth}{\centering\bfseries\fontsize{16.5}{20}\selectfont #1}%
    }%
  }%
}
...
\newcommand{\highlightbox}[2]{%
  ...
  \fcolorbox{PosterTeal}{PosterHighlight}{%
    \parbox{0.955\linewidth}{\fontsize{14}{17}\selectfont\textbf{#1} #2}%
  }%
}
...
\vspace{0.9em}
\takeawaybox{No single sentiment model dominates across all domains. Voting and selective routing to stronger models are one practical option for improving results.}
\vspace{1.2em}
\end{center}
]
...
This work compares classical ML, deep learning, transformers, and large language models across a multidomain benchmark. We test several voting variants and also analyze a selective hybrid pipeline that calls the more expensive model only when earlier stages are not confident enough.
...
\begin{itemize}[leftmargin=1.2em,itemsep=0.3em,topsep=0.3em]
  ...
  \item Can specific voting schemes and selective hybrid routing improve quality without relying on an expensive model for every sample?
\end{itemize}
...
\begin{tabular}{>{\raggedright\arraybackslash}p{0.20\linewidth} >{\centering\arraybackslash}p{0.12\linewidth} >{\centering\arraybackslash}p{0.13\linewidth} >{\centering\arraybackslash}p{0.13\linewidth} >{\centering\arraybackslash}p{0.13\linewidth} >{\centering\arraybackslash}p{0.16\linewidth}}
\toprule
\headerrow
\textbf{Dataset} & \textbf{Best single} & \llmstrategy{LLM-only} & \crossstrategy{Majority} & \cascadestrategy{Conf-casc} & \crossstrategy{Maj. no LLM} \\
\midrule
Finance-66 & 0.884 & 0.880 & 0.908 & \textbf{0.912} & 0.856 \\
Finance-75 & 0.938 & 0.912 & 0.960 & \textbf{0.964} & 0.907 \\
Finance-All & 0.971 & 0.934 & 0.976 & \textbf{0.980} & 0.916 \\
IMDB & 0.958 & \textbf{0.959} & \textbf{0.959} & \textbf{0.959} & 0.930 \\
Twitter & 0.866 & 0.556 & 0.863 & 0.870 & \textbf{0.891} \\
Yelp-Full & 0.666 & \textbf{0.660} & 0.652 & 0.659 & 0.594 \\
\bottomrule
\end{tabular}
}
\end{center}

The voting comparison shows that LLM-only aggregation can help on selected datasets, but confidence-weighted cross-model voting is the most reliable option overall. The clearest exception is Twitter, where majority voting without LLMs performs best because the three generative models underperform the CNN and LSTM baselines on that dataset.
...
\highlightbox{Scientific contribution:}{The study combines a unified multidomain comparison of five model families with a practical evaluation of three hybrid strategies and an error-overlap analysis that explains when aggregation can and cannot help.}
